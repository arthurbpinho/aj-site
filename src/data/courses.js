// Mapeamento de imagens em /public/midias/gruposcursos/ para cada curso/grupo
// e respectivos links Hotmart para compra avulsa.

export const courses = [
  {
    id: "biblia-e-psique",
    title: "A Bíblia e a Psique",
    subtitle: "Simbolismo da individuação no Antigo Testamento",
    author: "Arthur Bernardes",
    image: "/midias/gruposcursos/bibliapsique.png",
    description:
      "Um estudo aprofundado sobre as narrativas do Antigo Testamento à luz da Psicologia Analítica, percorrendo símbolos, arquétipos e o processo de individuação que atravessa os textos sagrados.",
    link: "https://pay.hotmart.com/Q100378659P",
    tag: "Curso",
  },
  {
    id: "voz-e-tempo",
    title: "A Voz e o Tempo",
    subtitle: "Reflexões para Jovens Terapeutas",
    author: "Arthur Bernardes",
    image: "/midias/gruposcursos/vozeotempo.png",
    description:
      "Reflexões essenciais para quem inicia a clínica junguiana: ética, presença, escuta e o lugar do terapeuta diante das demandas contemporâneas.",
    link: "https://pay.hotmart.com/W101374635P",
    tag: "Curso",
  },
  {
    id: "jung-literatura",
    title: "Carl Jung e Literatura",
    subtitle: "Quando a psique encontra a arte",
    author: "Henrique Barçante",
    image: "/midias/gruposcursos/carljungliteratura.png",
    description:
      "Oito semanas mergulhando nos textos de Jung sobre a obra de arte poética, além de produções de Marie-Louise von Franz e Sonu Shamdasani, atravessando obras literárias fundamentais.",
    link: "https://pay.hotmart.com/P104351329I",
    tag: "Curso",
  },
  {
    id: "tipos-psicologicos",
    title: "Compreendendo os Tipos Psicológicos",
    subtitle: "Minicurso",
    author: "João de Bragança",
    image: "/midias/gruposcursos/tipospsicologicos.png",
    description:
      "Introdução clara e rigorosa à tipologia junguiana: funções racionais e irracionais, atitudes extrovertida e introvertida, e o papel das funções na orientação da consciência.",
    link: "https://pay.hotmart.com/E97001018H",
    tag: "Minicurso",
  },
  {
    id: "fundamentos-psicologia-complexa",
    title: "Fundamentos da Psicologia Complexa",
    subtitle: "Pilares da obra junguiana",
    author: "Equipe Academia Junguiana",
    image: "/midias/gruposcursos/fundamentospsicomplexa.png",
    description:
      "Os conceitos fundamentais da Psicologia Analítica em uma trilha de estudos densa e didática: psique, inconsciente, complexos, arquétipos, símbolo e individuação.",
    link: "https://pay.hotmart.com/R101374480Y",
    tag: "Curso",
  },
  {
    id: "como-entender-sonhos",
    title: "Como Entender os Sonhos",
    subtitle: "Grupo de Discussão",
    author: "Arthur Bernardes",
    image: "/midias/gruposcursos/comoentenderossonhos.png",
    description:
      "Encontros de discussão sobre a abordagem junguiana dos sonhos: amplificação, contexto, séries oníricas e o uso clínico do material onírico.",
    link: "https://pay.hotmart.com/V95141695A",
    tag: "Grupo",
  },
  {
    id: "presente-futuro",
    title: "Presente e Futuro",
    subtitle: "Grupo de Discussão — O.C. X/I",
    author: "Equipe Academia Junguiana",
    image: "/midias/gruposcursos/presentefuturo.png",
    description:
      "Leitura compartilhada do volume X/I das Obras Completas, percorrendo as preocupações de Jung sobre massa, indivíduo e o destino simbólico do homem moderno.",
    link: "https://pay.hotmart.com/X105112647P",
    tag: "Grupo",
  },
  {
    id: "harry-potter",
    title: "Harry Potter e Psicologia Analítica",
    subtitle: "Símbolos, sombra e individuação na saga",
    author: "Equipe Academia Junguiana",
    image: "/midias/gruposcursos/harrypotter.png",
    description:
      "Uma travessia pela saga de Harry Potter sob a lente junguiana: arquétipos, sombra, anima, processo de individuação e os símbolos que atravessam toda a narrativa.",
    link: "https://pay.hotmart.com/L105112573R",
    tag: "Curso",
  },
  {
    id: "homem-e-seus-simbolos",
    title: "O Homem e Seus Símbolos",
    subtitle: "Grupo de Estudos",
    author: "Equipe Academia Junguiana",
    image: "/midias/gruposcursos/homemeseussimbolos.png",
    description:
      "Estudo guiado do clássico de Jung e seus colaboradores, capítulo a capítulo, conectando teoria, símbolos e a prática clínica.",
    link: "https://pay.hotmart.com/B105114199I",
    tag: "Grupo",
  },
  {
    id: "nise-machado",
    title: "Nise, Machado de Assis e Psicologia",
    subtitle: "Cultura brasileira sob a lente junguiana",
    author: "Equipe Academia Junguiana",
    image: "/midias/gruposcursos/nisemachadodeassiseapsi.png",
    description:
      "Diálogo entre Nise da Silveira, Machado de Assis e a Psicologia Analítica — uma leitura simbólica da cultura brasileira e de seus criadores.",
    link: "https://pay.hotmart.com/L102922969Y",
    tag: "Curso",
  },
  {
    id: "numinoso-e-religiao",
    title: "O Numinoso e a Religião",
    subtitle: "Na Psicologia Analítica",
    author: "Arthur Bernardes",
    image: "/midias/gruposcursos/numinosoeareligiao.jpg",
    description:
      "Cinco módulos atravessando do pensamento de Mircea Eliade à elaboração junguiana da experiência religiosa: o sagrado, a função transcendente e os símbolos coletivos.",
    link: "https://pay.hotmart.com/J102659484M",
    tag: "Curso",
  },
  {
    id: "psicologia-analitica-sonhos-2023",
    title: "Psicologia Analítica e Sonhos",
    subtitle: "Edição 2023",
    author: "Equipe Academia Junguiana",
    image: "/midias/gruposcursos/psianaliticaesonhos2023.png",
    description:
      "Gravação completa do curso de 2023 sobre o tratamento junguiano dos sonhos: teoria, método e vinhetas clínicas.",
    link: "https://pay.hotmart.com/H95173259Y",
    tag: "Curso",
  },
  {
    id: "experiencia-junguiana",
    title: "A Experiência Junguiana",
    subtitle: "Trilha introdutória",
    author: "Equipe Academia Junguiana",
    image: "/midias/gruposcursos/experienciajunguiana.png",
    description:
      "Porta de entrada à Psicologia Analítica — uma vivência didática que apresenta os principais conceitos e o estilo de estudo da Academia.",
    link: "https://pay.hotmart.com/X100137473B",
    tag: "Curso",
  },
];
